...
`ℤ^d`" is a `Finset` sum over `LatticeProb.originBox d L` for any `L` large enough, and the
product structure of `LatticeProb.K` turns it into a product of one-dimensional sums
...
/-- Identifies the sum of the product kernel over a finite originBox with a product of one-dimensional sums, the reduction underlying the per-step estimates in Section 8. -/
lemma originBox_eq (d L : ℕ) :
    originBox d L = Fintype.piFinset (fun _ : Fin d => Finset.Icc (-(L : ℤ)) (L : ℤ)) := rfl
...
/-- Upper-bounds the total weight of the product kernel over a finite originBox by a product of one-dimensional sums, giving the constant behind the per-step estimates in the proof of the green lemma. -/
lemma sum_K_box {L : ℕ} (n : Fin d → ℕ) (h : ∀ i, n i ≤ L) :
    ∑ x ∈ originBox d L, K n x = 1 := by
  rw [originBox_eq]
  simp only [K]
...
/-- Bounds the sum of the absolute value of the product kernel over a finite originBox, providing the finite-volume estimate underlying the per-step bounds in the proof of the green function lemma. -/
lemma sum_abs_diff_K_box {L : ℕ} (n : Fin d → ℕ) (i₀ : Fin d) (h : ∀ i, n i + 1 ≤ L) :
    ∑ x ∈ originBox d L, |K n (x + dirVec ((i₀, true) : Dir d)) - K n x| = 2 * P1 (n i₀) 0 := by
  classical
  ...
  rw [Finset.sum_congr rfl fun x _ => hpt x, originBox_eq,
