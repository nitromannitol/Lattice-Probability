...
inequality" (`sec:insertion`), the paper.
...
/-- The Dirichlet Laplacian of a finite set, the paper:
...
/-- `Δ_A` is invertible for `d ≥ 1`, the paper:
...
/-- The mean exit time `u_A`, the paper:
