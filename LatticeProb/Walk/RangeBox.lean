...
the path model (`eq:range-originBox`):
...
lemma mem_originBox_of_norm_le {r : ℕ} {x : Site d} (hx : ‖x‖ ≤ (r : ℝ)) :
...
/-- `eq:range-originBox`: the range at time `k` lies in the box of radius `r`, so it
has at most `(2r+1)^d` sites. -/
theorem card_pathRange_le_originBox (w : Fin n → Dir d) (k r : ℕ)
    (hr : ∀ j ≤ k, ‖pos w j‖ ≤ (r : ℝ)) :
    (pathRange w k).card ≤ (2 * r + 1) ^ d := by
  classical
  have hsub : pathRange w k ⊆ Fintype.piFinset (fun _ : Fin d => Finset.Icc (-(r : ℤ)) (r : ℤ)) := by
    intro x hx
    obtain ⟨j, hj, rfl⟩ := Finset.mem_image.mp hx
    exact mem_originBox_of_norm_le (hr j (Nat.lt_succ_iff.mp (Finset.mem_range.mp hj)))
  calc (pathRange w k).card
      ≤ (Fintype.piFinset (fun _ : Fin d => Finset.Icc (-(r : ℤ)) (r : ℤ))).card :=
        Finset.card_le_card hsub
    _ = (2 * r + 1) ^ d := card_originBox r
