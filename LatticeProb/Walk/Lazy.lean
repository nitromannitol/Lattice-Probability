...
* Convention W-001 (`Q` is an operator, not a matrix).  The paper writes `Q^r(0,x)`
  ...
  `Q^r(0, ·)`.  `LatticeProb.Q_eq_pushforward` justifies the identification: the `2d`
...
noncomputable def originBox (d k : ℕ) : Finset (Site d) :=
...
theorem exists_of_notMem_box {k : ℕ} {x : Site d} (hx : x ∉ originBox d k) :
    ∃ i : Fin d, (k : ℤ) < |x i| := by
  by_contra hc
  push Not at hc
...
theorem summable_gR_gradient (R : ℕ) :
  ...
  refine summable_of_ne_finset_zero
    (s := originBox d (R + 1) ×ˢ (Finset.univ : Finset (Fin d))) ?_
  intro p hp
