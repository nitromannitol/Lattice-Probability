...
Section 2, the paper (label `lem:schur`).
...
lemma harmonic_sum (hd : 0 < d) (A : Finset (Site d)) (p : Site d) (hp : p ∉ A)
    ...
    ∑ y ∈ A, f y = f p * (LatticeProb.h A p - 1) := by
  classical
  ...
  have hsum : ∑ y : A, ((dirichletLaplacian A)⁻¹ *ᵥ χ) y = -(LatticeProb.h A p - 1) := by
...
lemma one_le_h (hd : 0 < d) (A : Finset (Site d)) (x : Site d) : 1 ≤ LatticeProb.h A x := by
...
/-- The Green function on the finite originBox is nonpositive, providing the sign control needed for the comparison argument in the one-point insertion lemma. -/
...
theorem one_point_insertion (hd : 1 ≤ d)
    ...
    u (insert x A) x = Reff A x * LatticeProb.h A x ∧
      T (insert x A) - T A = LatticeProb.h A x * u (insert x A) x ∧
      1 / (2 * (d : ℝ)) ≤ Reff A x := by
  classical
  have hd0 : 0 < d := hd
  ...
  have hfirst : u (insert x A) x = Reff A x * LatticeProb.h A x := by
  ...
  have hsecond : T (insert x A) - T A = LatticeProb.h A x * u (insert x A) x := by
    rw [hTsplit, T, ← hB]
