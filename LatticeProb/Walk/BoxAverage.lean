...
lemma sum_u_le_T (hd : 0 < d) (A S : Finset (Site d)) :
  ...
  have hsplit : ∑ z ∈ S ∩ A, u A z + ∑ z ∈ S \ A, u A z = ∑ z ∈ S, u A z :=
    Finset.sum_inter_add_sum_sdiff S A (u A)
...
/-- Every site of A lies in the middle layer of the enlarged set, a bookkeeping step used when comparing the walk's behaviour on A with its behaviour on the surrounding originBox. -/
...
/-- The originBox `{0,…,q}^d` of side `q+1`, as a set of shift vectors. -/
noncomputable def shiftBox (d q : ℕ) : Finset (Site d) := qbox (0 : Site d) q

/-- The shifted originBox used to average the exit-time potential has exactly the cardinality of the side-length originBox, matching the |Q| ≥ 2|A| requirement of the comparison argument. -/
lemma card_shiftBox (d q : ℕ) : (shiftBox d q).card = (q+1)^d := card_qbox 0 q

/-- Bounds the ℓ¹ norm of any site in the shifted originBox by d·q, which is the diameter estimate feeding into the Poincaré comparison between the mean exit time and its box average. -/
